...
float read_adc_voltage(unsigned char channel) {
    ...
    voltage = ((raw_value * 5) / 1023.0 )*100; // this is going to change because we're dealing with a sensor and not a regular voltage, but first i'll try a AN0 for a start
    return voltage;
